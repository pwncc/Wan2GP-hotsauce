...
def parse_wgp_args(family_handlers: Sequence[str], config_filename: str, default_lora_root: str, argv: Sequence[str] | None = None):
    ...
    add("--multi-gpu-offload", action="store_true", help="Use secondary CUDA GPUs as an offload cache before falling back to RAM/disk-backed weights")
    add("--multi-gpu-offload-devices", type=str, default="", help="Comma-separated secondary CUDA devices for offload cache, or auto")
    add("--multi-gpu-offload-vram-ratio", type=float, default=0.80, help="Fraction of free secondary-GPU VRAM available for offload cache")
    add("--disable-ram-weight-cache", action="store_true", help="Do not materialize disk-backed offload weights into system RAM")
